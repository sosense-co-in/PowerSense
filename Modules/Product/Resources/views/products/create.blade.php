...
@push('page_scripts')
    ...
    <script>
        $(document).ready(function() {
            var currencySettings = {!! json_encode(settings()->currency) !!};

            $('#product_cost').maskMoney({
                prefix: currencySettings.symbol || '₹', // Fallback to ₹ if undefined
                thousands: currencySettings.thousand_separator || ',',
                decimal: currencySettings.decimal_separator || '.'
            });
            ...
            $('#product-form').submit(function() {
                var product_cost = $('#product_cost').maskMoney('unmasked')[
                0]; // Gets the unmasked value (numeric)
                var product_price = $('#product_price').maskMoney('unmasked')[
                0]; // Gets the unmasked value (numeric)
                $('#product_cost').val(product_cost); // Sets the input field to the numeric value
                $('#product_price').val(product_price); // Sets the input field to the numeric value
            });
        });
    </script>
@endpush
